...
function renderPrioritySheet() {
  ...
  QBANK.tiers.forEach(tier => {
    ...
    // Sidebar tier group
    let sidebarTierEl = null;
    if (sidebarNav) {
      sidebarTierEl = document.createElement('div');
      sidebarTierEl.className = 'sidebar-tier';
      sidebarTierEl.dataset.tier = tier.id;
      sidebarTierEl.innerHTML =
        '<div class="sidebar-tier-label"><span class="sidebar-tier-dot"></span>' + esc(tier.title) + '</div>';
      sidebarNav.appendChild(sidebarTierEl);
    }
    ...
    sections.forEach(section => {
      ...
      if (sidebarTierEl) {
        ...
        sa.className = 'sidebar-link';
        sa.href = '#section-' + section.id;
        sa.dataset.sectionId = section.id;
        sa.textContent = section.label;
...
function renderTopicSection(section, tier) {
  ...
  wrap.innerHTML =
    '<div class="topic-section-header">' +
      '<span class="topic-section-title">' + esc(section.label) + '</span>' +
...
function renderCard(card, section) {
  ...
  body.className = 'topic-body collapsed';
  ...
  if (card.guide) {
    ...
    btn.className = 'guide-toggle';
    btn.textContent = '📋 Pattern template';
    const guideBody = document.createElement('div');
    guideBody.className = 'guide-body collapsed';
    guideBody.innerHTML =
      '<div class="guide-signal"><strong>When to use:</strong> ' + esc(card.guide.signal) + '</div>' +
      '<pre class="guide-code">' + esc(card.guide.template) + '</pre>';
...
function updateCardProgress(topic) {
  ...
  const done = rows.filter(r => !r.classList.contains('status-not-attempted')).length;
...
function updatePriorityTotal() {
  ...
  const done = counts.mastered + counts.revision + counts.failed;
...
function setupSidebarObserver() {
  ...
  const linkMap = {};
  links.forEach(a => { linkMap[a.dataset.sectionId] = a; });
  ...
  _sidebarObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        ...
        // scroll the sidebar link into view quietly
